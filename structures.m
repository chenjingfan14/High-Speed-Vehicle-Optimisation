function structures(wing, points)
...
xSkinUpOut = squeeze(skin.Upper.x(:,1,:));
xSkinLoOut = squeeze(skin.Lower.x(:,1,:));
xSkinUpIn = squeeze(skin.Upper.x(i,2,:));
xSkinLoIn = squeeze(skin.Lower.x(i,2,:));

ySkinUpOut = squeeze(skin.Upper.y(:,1,:));
ySkinLoOut = squeeze(skin.Lower.y(:,1,:));
ySkinUpIn = squeeze(skin.Upper.y(i,2,:));
ySkinLoIn = squeeze(skin.Lower.y(i,2,:));

zSkinUpOut = squeeze(skin.Upper.z(:,1,:));
zSkinLoOut = squeeze(skin.Lower.z(:,1,:));
zSkinUpIn = squeeze(skin.Upper.z(i,2,:));
zSkinLoIn = squeeze(skin.Lower.z(i,2,:));
...
az = squeeze(zMidUpper(1:end-1,:));
bz = squeeze(zMidUpper(2:end,:));
cz = squeeze(zMidLower(2:end,:));
dz = squeeze(zMidLower(1:end-1,:));
...
spars = wing.Spars.Points;

nSpars = numel(spars);
nSecs = wing.Partitions + 1;
sparPerSec = nSpars/nSecs;

sparThick = wing.Spars.Thickness;
...
sparMidDiff = sparMid(:,end-2:end) - sparMid(:,1:3);
...
% Front and back verticle lines of spars
% sparFrontDiff = squeeze(diff(sparFront))';
% sparBackDiff = squeeze(diff(sparBack))';
...
xOuterSparCentroid = xSparCentroid([1 end],:);
yOuterSparCentroid = ySparCentroid([1 end],:);
zOuterSparCentroid = zSparCentroid([1 end],:);
